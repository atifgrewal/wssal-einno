        ...
        <div class="col-md-12">
            @can('attributedetail_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.attributedetails.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $attributedetail->id }}">
                                        ...
                                        <td>
                                            @can('attributedetail_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.attributedetails.show', $attributedetail->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('attributedetail_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.attributedetails.edit', $attributedetail->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('attributedetail_delete')
                                                <form action="{{ route('frontend.attributedetails.destroy', $attributedetail->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('attributedetail_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.attributedetails.massDestroy') }}",
