...
<div class="form-group">
    ...
    <input class="form-control {{ $errors->has('st_date') ? 'is-invalid' : '' }}" type="date" name="st_date" id="st_date" value="{{ old('st_date', '') }}" step="0.01" required>
...
<div class="form-group">
    ...
    <select class="form-control {{ $errors->has('order_type') ? 'is-invalid' : '' }}" name="order_type" id="order_type" required>
        ...
        @foreach(App\Models\Order::ORDER_TYPE_SELECT as $key => $label)
            <option value="{{ $key }}" {{ old('order_type', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
        @endforeach
    </select>
    ...
    <span class="help-block">{{ trans('cruds.order.fields.order_type_helper') }}</span>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('payment') ? 'is-invalid' : '' }}" type="number" name="payment" id="payment" value="{{ old('payment', '') }}" step="0.01" required>
